...
class ApiFlow(unittest.TestCase):
    ...
    def test_readSQLcase(self):
        sql="SELECT id,`apiname`,apiurl,apimethod,apiparamvalue,apiresult,`apistatus` from apitest_apistep where id=2 "
...
def readSQLcase():
    """读取数据库中相应的接口用例数据"""
    sql="SELECT id,`apiname`,apitesturl,apimethod,apiparamvalue,apiresult,`apistatus`from apitest_apistep where id=3"
...
def interfaceTest(case_list):
    ...
    for case in case_list:
        ...
        if param== '':
            ...
            new_url = 'https://dcd.cmanuf.com'+url
        ...
        if method.upper()=='PUT':
            ...
            'CredentialId':id, 'Content-Type': 'application/json'}
            body_data=param
        ...
        if method.upper()=="POST":
            ...
            if "=" in urlParam(param):
                ...
                print(case_id,new_url,urlParam(param))
                print('case_id is:'+str(case_id)+' request is ' +new_url+' bodyis '+urlParam(param))
...
def writeBug(bug_id,interface_name,request,response,res_check):
    ...
    bugdetail = '[请求数据]<br />'+request+'<br/>'+'[预期结果]<br/>'+res_check+'<br/>'+'<br/>'+'[响应数据]<br />'+'<br/>'+response
    print(bugdetail)
    sql = "INSERT INTO `bug_bug` (`bugname`,`bugdetail`,`bugstatus`,`bug_leval`, `bugcreater`,`bugassign`,`create_time`,`update_time`,`Product_id`) VALUES ('%s','%s','1','1','王','邹辉', '%s','%s','2');"%(bugname,bugdetail,now,now)
    # print('sql\n',sql)
    coon =pymysql.connect(user='root',passwd='123456',db='autotest',port=3306,host='127.0.0.1',charset='utf8')
    cursor = coon.cursor()
    cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    testunit = unittest.TestSuite()#创建测试集合
    testunit.addTest(ApiFlow("test_readSQLcase"))#向测试集合增加测试用例
    filename='D:\\C\\'+now+'_'+"apitest_report.html"
    fp=open(filename,'wb')
    print(fp)
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title="流程接口测试报告",description="流程场景接口")
